...
lazy_static! {
    static ref MOCK_HOST_KV_STORE: Mutex<HashMap<Arc<Vec<u8>>, Arc<Vec<u8>>>> =
        Mutex::new(HashMap::new());
}

pub fn set(key: Arc<Vec<u8>>, value: Arc<Vec<u8>>) {
    let mut store = MOCK_HOST_KV_STORE.lock().unwrap();
    store.insert(key, value);
}

pub fn get(key: Arc<Vec<u8>>) -> Arc<Vec<u8>> {
    let store = MOCK_HOST_KV_STORE.lock().unwrap();
